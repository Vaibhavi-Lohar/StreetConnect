...
// Fetch supplier's full name from `users` table
...
if ($result->num_rows === 1) {
    ...
    $supplier_name = $username; // fallback
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $products[] = [
            ...
            'stock' => 0, // You can add stock column in DB if needed
            'unit' => $row['unit'],
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="text-sm text-gray-600">
                            <span>Welcome back, <strong><?php echo htmlspecialchars($supplier_name); ?></strong></span>

                        </div>
                        <a href="edit-profile.php">
                            <div class="w-8 h-8 bg-fresh-green rounded-full flex items-center justify-center cursor-pointer">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-white rounded-xl shadow-md p-6">
                            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
                                ...
                                <button onclick="openAddProductModal()" class="bg-fresh-green text-white px-6 py-2 rounded-lg hover:bg-deep-green transition-colors duration-200 flex items-center space-x-2 shadow-md">
...
<div class="bg-white rounded-xl shadow-md overflow-hidden mt-6">
        ...
        <table class="w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="text-left py-4 px-6 font-medium text-gray-600">Track</th>
                </tr>
            </thead>
            <tbody id="orders-table">
                <?php
                while ($row = $result->fetch_assoc()):
                    $payment = isset($row['payment_method']) ? $row['payment_method'] : 'N/A';
                    $status = isset($row['order_status']) ? $row['order_status'] : 'N/A';
                ?>
                <tr class="border-b border-gray-100 hover:bg-gray-50">
                    <td class="py-4 px-6"><?= htmlspecialchars($row['order_id']) ?></td>
                    <td class="py-4 px-6"><?= htmlspecialchars($row['customer_name']) ?></td>
                    <td class="py-4 px-6 text-green-700 font-semibold">₹<?= number_format($row['total_amount'], 2) ?></td>
                    <td class="py-4 px-6"><?= htmlspecialchars($payment) ?></td>
                    <td class="py-4 px-6">
                        <span class="inline-block px-2 py-1 text-xs rounded bg-blue-100 text-blue-700"><?= htmlspecialchars($status) ?></span>
                    </td>
                    <td class="py-4 px-6"><?= htmlspecialchars($row['created_at']) ?></td>
                    <td class="py-4 px-6">
                        <button onclick="trackLocation('<?= $row['delivery_latitude'] ?>', '<?= $row['delivery_longitude'] ?>')" class="text-blue-600 hover:underline">
                            Track
                        </button>
                    </td>
                </tr>
        ...
        <div class="bg-white rounded-xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-bold text-gray-800 flex items-center">
                    <i class="fas fa-plus-circle text-fresh-green mr-2"></i>
                    Add New Product
                </h2>
                <button onclick="closeAddProductModal()" class="text-gray-400 hover:text-gray-600 text-xl">
            ...
            <form id="add-product-form" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="product-name" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-fresh-green focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Category *</label>
                        <select id="product-category" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-fresh-green focus:border-transparent">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="number" id="product-price" required min="0" step="0.01" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-fresh-green focus:border-transparent">
                    </div>
                                        <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Unit *</label>
                        <select id="product-unit" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-fresh-green focus:border-transparent">
                    ...
                    <div>
                        ...
                        <input type="file" id="product-image" required min="0" accept="image*/" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-fresh-green focus:border-transparent">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Description</label>
                    <textarea id="product-description" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-fresh-green focus:border-transparent" placeholder="Enter product description..."></textarea>
                </div>
                <div class="flex space-x-4 pt-4">
                    <button type="submit" class="flex-1 bg-fresh-green text-white py-3 px-4 rounded-lg hover:bg-deep-green transition-colors duration-200 font-medium">
                        <i class="fas fa-plus mr-2"></i>Add Product
                    </button>
                    <button type="button" onclick="closeAddProductModal()" class="flex-1 bg-gray-300 text-gray-700 py-3 px-4 rounded-lg hover:bg-gray-400 transition-colors duration-200 font-medium">
                        Cancel
                    </button>
                </div>
            </form>
    ...
    <div id="payment-test-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 hidden">
        <div class="bg-white rounded-xl max-w-md w-full p-6">
            ...
            <!-- Payment Button -->
            <button id="razorpay-payment-button" onclick="startPayment(494)" class="w-full bg-fresh-green hover:bg-deep-green text-white font-semibold py-4 px-6 rounded-lg transition-colors duration-200 flex items-center justify-center space-x-2">
    ...
    <script>
        // Sample Data Storage
        let products = <?php echo json_encode($products, JSON_PRETTY_PRINT); ?>;
...
window.addEventListener('DOMContentLoaded', () => {
  fetch("get-payments.php")
    ...
    .then(data => {
      ...
      const statusBadge = {
        Success: `<span class="px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800"><i class="fas fa-check mr-1"></i>Success</span>`,
        Pending: `<span class="px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800"><i class="fas fa-clock mr-1"></i>Pending</span>`,
        Failed: `<span class="px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800"><i class="fas fa-times mr-1"></i>Failed</span>`
      };

      data.forEach(row => {
        tbody.innerHTML += `
          <tr class="border-b border-gray-100 hover:bg-gray-50">
            <td class="py-3 px-4 font-mono text-sm text-gray-800">${row.transaction_id}</td>
            <td class="py-3 px-4 font-medium text-gray-800">${row.order_id}</td>
            <td class="py-3 px-4 text-gray-600">${row.customer_name}</td>
            <td class="py-3 px-4 font-semibold text-fresh-green">₹${row.amount}</td>
            <td class="py-3 px-4">
              <div class="flex items-center space-x-2">
                <i class="fas fa-credit-card text-blue-500"></i>
                <span class="text-sm text-gray-600">${row.method}</span>
              </div>
            </td>
            <td class="py-3 px-4">${statusBadge[row.status] || row.status}</td>
            <td class="py-3 px-4 text-gray-600">${row.date}</td>
            <td class="py-3 px-4">
...
const RAZORPAY_CONFIG = {
  ...
  try {
    ...
    const createProduct = (imageDataURL) => {
        const newProduct = {
            id: Date.now(),
            ...
            image: imageDataURL || 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=300&h=200&fit=crop'
        };
    ...
    if (file) {
        ...
        reader.onload = function () {
            createProduct(reader.result);  // use uploaded image
        };
        reader.readAsDataURL(file);
    } else {
        createProduct(null);  // use default image
    }
});
        ...
        function handleLogout() {
            if (confirm('Are you sure you want to logout? Any unsaved changes will be lost.')) {
                ...
                setTimeout(() => {
                     ...
                     window.location.href = '/streetfood/index.php';
                }, 1000);
            }
        }
